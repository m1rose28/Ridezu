...
function fillRide($driverrideid)
{
    ...
    try {
        ...
        $stmt1->bindParam("amount", $ride->nextamount);
...
function cancelRide($rideid,$fbid)
{
	...
	if ($ridedata->eventtype=='0') //driver
	{
		...
		if (($ridedata->refrideid != null) && ($ridedata->refrideid !=''))
		{
			...
			$sql2= "update transhistory set eventstate='REQUEST',refrideid=null where rideid in (:riderrideid) and eventtype='1' and eventtime > :currentdatetime";
			$stmt2=$db->prepare($sql2);
	...
	else if ($ridedata->eventtype=='1') //rider
	{
		...
		if (($ridedata->refrideid != null)&& ($ridedata->refrideid !=''))
		{
		 ...
		 $driverridedata = $stmt3->fetchObject();
		  ...
		  //$sql4="update transhistory set eventstate='ACTIVE' where rideid=:driverrideid and eventtype='0'and eventtime > :currentdatetime";
